...
int main(int argc, char **argv)
{
    ...
    std::vector<float> massQ(opt.nq * opt.d);
    {
        ...
        readXvecFvec<int8_t>(query_input, massQ.data(), opt.d, opt.nq);
    }
    //==================
    // Initialize Index
    //==================
    ...
    if (exists(opt.path_pq) && exists(opt.path_norm_pq)) {
        ...
        std::vector<float> trainvecs(opt.nt * opt.d);
        {
            ...
            readXvecFvec<int8_t>(learn_input, trainvecs.data(), opt.d, opt.nt);
        }
    ...
    if (!exists(opt.path_precomputed_idxs)){
        ...
        for (size_t i = 0; i < nbatches; i++) {
            ...
            readXvecFvec<int8_t>(input, batch.data(), opt.d, batch_size);
    ...
    if (exists(opt.path_index)){
        ...
        std::vector<idx_t> ids_batch(basic_batch_size);

        for (size_t b = 0; b < nbatches; b++) {
            uint32_t batch_size = b == nbatches - 1 ? (opt.nb - b * basic_batch_size) : basic_batch_size;

            if (b % 10 == 0) {
                std::cout << "[" << stopw.getElapsedTimeMicro() / 1000000 << "s] " << (100. * b) / nbatches << "%\n";
            }
            readXvec<idx_t>(idx_input, idx_batch.data(), batch_size, 1);
            readXvecFvec<int8_t>(base_input, batch.data(), opt.d, batch_size);

            for (size_t i = 0; i < batch_size; i++)
                ids_batch[i] = basic_batch_size * b + i;
